...
const DataGrid: React.FunctionComponent<Props> = (p) => {
  ...
  const getBoundsForItem = React.useCallback(
    (
      ...
      if (row === undefined) {
        ...
        result.height =
          (typeof rowHeight === "number" ? rowHeight : rowHeight(row)) + 1;
      }
    ...
    (
      ...
      if (
        ...
        result = {
          kind: "out-of-bounds",
          location: [
            col !== -1 ? col : x < 0 ? 0 : columns.length - 1,
            row ?? rows - 1,
          ],
  ...
  function isSameItem(
    item: GridMouseEventArgs | undefined,
    other: GridMouseEventArgs | undefined
  ) {
  ...
  const draw = React.useCallback(() => {
    ...
    const getRowHeight = (r: number) =>
      typeof rowHeight === "number" ? rowHeight : rowHeight(r);
    ...
    if (damage !== undefined) {
      ...
      while (y < height) {
        ...
        for (const c of effectiveCols) {
          const rowLocal = row;
          if (
            damage.find((d) => d[0] === c.sourceIndex && d[1] === rowLocal) !==
            undefined
          ) {
    ...
    if (!blitted && damage === undefined) {
      ...
      for (const c of effectiveCols) {
        ...
        const hovered =
          hoveredCol === c.sourceIndex && dragAndDropState === undefined;

        const hasSelectedCell =
          selectedCell !== undefined && selectedCell.cell[0] === c.sourceIndex;
        ...
        if (
          ...
          for (const c of effectiveCols) {
            ...
            if (
              damage !== undefined &&
              damage.find(
                (d) => d[0] === c.sourceIndex && d[1] === rowLocal
              ) === undefined
            ) {
              x += c.width;
              continue;
            ...
            let highlighted =
              rowSelected ||
              selectedColumns?.includes(c.sourceIndex) ||
              isFocused;
            ...
            if (highlighted) {
              ...
              if (
                prelightCells?.find(
                  (pre) => pre[0] === c.sourceIndex && pre[1] === row
                ) !== undefined
              ) {
  ...
  const headerHovered =
    hoveredItem?.[0] !== undefined && hoveredItem[1] === undefined;
  ...
  const onMouseMoveImpl = React.useCallback(
    (ev: MouseEvent) => {
      ...
      if (!isSameItem(args, hoveredRef.current)) {
        onItemHovered?.(args);
        setHoveredItem(
          args.kind === "out-of-bounds" ? undefined : args.location
        );
  ...
  const onDragStartImpl = React.useCallback(
    (event: React.DragEvent<HTMLCanvasElement>) => {
      ...
      const args = getMouseArgsForPosition(
        canvas,
        event.clientX,
        event.clientY
      );
