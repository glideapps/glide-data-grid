...
export interface DataGridSearchProps
  extends Subtract<ScrollingDataGridProps, Handled> {
...
const DataGridSearch: React.FunctionComponent<DataGridSearchProps> = (p) => {
  ...
  const rowsSearchedProgress = Math.floor(
    ((searchStatus?.rowsSearched ?? 0) / rows) * 100
  );
  ...
  if (searchStatus !== undefined) {
    resultString = `${searchStatus.results} result${
      searchStatus.results !== 1 && "s"
    }`;
