...
interface Handled {
  ...
  readonly onSearchResultsChanged?: (
...
const DataEditor: React.FunctionComponent<DataEditorProps> = (p) => {
  ...
  const onMouseUp = React.useCallback(
    (args: GridMouseEventArgs) => {
      ...
      if (
        ...
        mouse?.previousSelection?.cell === undefined
      )
        return;
      const [col, row] = args.location;
  ...
  const updateSelectedCell = React.useCallback(
    (col: number, row: number): boolean => {
      ...
      if (col === selectedCell?.cell[0] && row === selectedCell?.cell[1])
        return false;
      setSelectedCell({ cell: [col, row] });
      ...
      if (scrollRef.current !== null) {
        ...
        const minXScrollToCol = (index: number) => {
          ...
          const availableSpace =
            clientWidth -
            (rowMarkers ? rowMarkerWidth : 0) -
            mangledCols[index].width;
          let offset = 0;
  ...
  const onFinishEditing = React.useCallback(
    (
      ...
      if (selectedCell !== undefined && (movX !== 0 || movY !== 0)) {
        updateSelectedCell(
          selectedCell.cell[0] + movX,
          selectedCell.cell[1] + movY
        );
      }
    },
    [
  ...
  const onKeyDown = React.useCallback(
    (event: GridKeyEventArgs) => {
      const fn = async () => {
        ...
        const isDeleteKey =
          event.key === "Delete" || (browserIsOSX && event.key === "Backspace");
  ...
  return (
    <>
      ...
      />
      {overlay !== undefined && (
        <DataGridOverlayEditor {...overlay} onFinishEditing={onFinishEditing} />
      )}
    </>
  );
};
