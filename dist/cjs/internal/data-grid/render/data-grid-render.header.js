...
function drawHeaderInner(ctx, x, y, width, height, c, selected, theme, isHovered, posX, posY, hoverAmount, spriteManager, touchMode, isRtl, headerLayout) {
    ...
    if (headerLayout.textBounds !== undefined) {
        ctx.fillText(c.title, isRtl ? headerLayout.textBounds.x + headerLayout.textBounds.width : headerLayout.textBounds.x, y + height / 2 + (0, data_grid_lib_js_1.getMiddleCenterBias)(ctx, theme.headerFontFull));
    }
    if (isRtl) {
        ctx.textAlign = "left";
    }
