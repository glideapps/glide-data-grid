...
test.describe('Selection', () => {
    ...
    test('Select & delete multiple row data', async ({ page }) => {
        ...
        await gridPage.clickOnCell(1,2);
        await gridPage.clickOnCell(1,3,{modifiers: ['Meta', 'Control']});
        await gridPage.clickOnCell(1,4,{modifiers: ['Meta', 'Control']});
        await page.keyboard.press('Delete');
    ...
    test('Selecct & delete multiple columns data', async ({ page }) => {
        ...
        const clickArgs = {position: {x: 10, y:0}};
        await gridPage.clickOnColumn(2, clickArgs);
        await gridPage.clickOnColumn(3,{modifiers: ['Meta', 'Control'], ...clickArgs});
        await gridPage.clickOnColumn(5,{modifiers: ['Meta', 'Control'], ...clickArgs});
    ...
    test('select cells range & delete', async ({page})=> {
        ...
        await gridPage.rangeSelect(2, 2, 5,5);
        await page.keyboard.press('Delete');
