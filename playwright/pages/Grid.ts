...
export class Grid extends Story {
    ...
    private getMergedPosition(originalPosition: ICellPosition, newPosition?: ICellPosition): ICellPosition {
        return {
    ...
    public async clickOnCell(cell: number, row: number, options?: Parameters<Locator["click"]>[0], double?: boolean) {
    ...
    public async dragColumns(sourceColumn: number, targetColumn: number) {
        ...
        return this.canvas.dragTo(this.canvas, {
            sourcePosition: this.getMergedPosition(sourcePosition, { x: COLUMN_CORNER_SAFE_OFFSET, y: 0 }),
            targetPosition: this.getMergedPosition(targetPosition, { x: COLUMN_CORNER_SAFE_OFFSET, y: 0 }),
            timeout: DEFAULT_PLAYWRIGHT_CLICK_TIMEOUT,
            force: true,
        });
    }
    ...
    public async fillHandle(sourceCol: number, sourceRow: number, targetCol: number, targetRow: number) {
        ...
        await this.canvas.dragTo(this.canvas, {
            sourcePosition: this.getMergedPosition(sourcePosition, { x: sourceColumnWidth - 1, y: this.rowHeight - 2 }),
            targetPosition,
            timeout: DEFAULT_PLAYWRIGHT_CLICK_TIMEOUT,
            force: true,
        });
