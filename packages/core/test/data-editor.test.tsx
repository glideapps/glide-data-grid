...
function scrollWidthMockImpl() {
    return basicProps.columns.map(c => (isSizedGridColumn(c) ? c.width : 150)).reduce((pv, cv) => pv + cv, 0);
}
...
describe("data-editor", () => {
    ...
    test("Copy/paste", async () => {
        ...
        const pasteSpy = jest.fn(
            (_target: any, _values: any, _clipboardData?: any, _groupId?: any, _startGroupRow?: any) => true
        );
        jest.useFakeTimers();
        render(
    ...
    test("Custom Copy", async () => {
        ...
        jest.useFakeTimers();
        render(
            <EventedDataEditor {...basicProps} onGridSelectionChange={spy} onCopy={onCopySpy} onPaste={onPasteSpy} />,
            {
                wrapper: Context,
            }
        );
        ...
        expect(onCopySpy).toHaveBeenCalledWith(
            [[expect.objectContaining({ displayData: "1, 0" }), expect.objectContaining({ displayData: "2, 0" })]],
            expect.anything()
        );
    });
    ...
    test("Copy/paste with grouping", async () => {
        ...
        expect(spy).toBeCalledWith(expect.objectContaining({ current: expect.objectContaining({ cell: [1, 4] }) }));
