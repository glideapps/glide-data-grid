...
const DataGridOverlayEditor: React.FunctionComponent<DataGridOverlayEditorProps> = (p) => {
  ...
  const [tempValue, setTempValueRaw] = React.useState<GridCell | undefined>(
    forceEditMode ? content : undefined
  );
  ...
  const [isValid, setIsValid] = React.useState(() => {
    ...
    return !(
      isEditableGridCell(content) && validateCell?.(cell, content, lastValueRef.current) === false
    );
  });
  ...
  const [keyboardEventHandlingMode, setKeyboardEventHandlingMode] =
    React.useState<KeyboardEventHandlingMode>('auto');
  ...
  const [editorProvider, useLabel] = React.useMemo(():
    | [ProvideEditorCallbackResult<GridCell>, boolean]
    | [] => {
  ...
  if (editorProvider !== undefined) {
    ...
    editor = (
      <CustomEditor
        ...
        validatedSelection={
          isEditableGridCell(targetValue) ? targetValue.selectionRange : undefined
        }
