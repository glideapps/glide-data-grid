...
export function measureColumn(
  ...
  if (selectedData !== undefined) {
    sizes.push(
      ...selectedData
        ...
        .map((cell) => measureCell(ctx, cell, theme, getCellRenderer))
    );
  }
  ...
  const final = Math.max(
    ...
    Math.min(Math.floor(maxColumnWidth), Math.ceil(biggest))
  );
...
export function useColumnSizer(
  ...
  React.useLayoutEffect(() => {
    ...
    const fn = async () => {
      ...
      const tailGetResult =
        tailRows > 0 ? getCells(tailComputeArea, abortController.signal) : undefined;
      let toSet: CellArray;
  ...
  return React.useMemo(() => {
    ...
    if (totalWidth < clientWidth && distribute.length > 0) {
      ...
      for (let di = 0; di < distribute.length; di++) {
        ...
        const toAdd =
          di === distribute.length - 1
            ? remaining
            : Math.min(remaining, Math.floor(extra * weighted));
        writeable[i] = {
          ...result[i],
          growOffset: toAdd,
