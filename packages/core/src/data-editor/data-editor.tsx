...
function shiftSelection(
    ...
    if (offset === 0 || (input.columns.length === 0 && input.current === undefined)) return { ...input, rows };

    let current;
    if (input.current !== undefined) {
        const cellLocation: Item = [input.current.cell[0] + offset, input.current.cell[1]];
        current = {
            cell: getMangledCellLocation === undefined ? cellLocation : getMangledCellLocation(cellLocation),
            range: {
                ...input.current.range,
...
export interface DataEditorProps extends Props {
     ...
     * @group Editing
     */
    readonly onDelete?: (selection: GridSelection, groupRows: GroupContentRow[]) => boolean | GridSelection;
     ...
     * @group Events
     */
    readonly onCellContextMenu?: (cellLocation: Item, event: CellClickedEventArgs, groupId?: string) => void;
...
const DataEditorImpl: React.ForwardRefRenderFunction<DataEditorRef, DataEditorProps> = (p, forwardedRef) => {
    ...
    const showTrailingBlankRow = onRowAppended !== undefined && !hasGroups && trailingRowOptions !== undefined;
    ...
    const onDelete = React.useCallback(
        ...
        [getMangledCellLocation, getGroupRowDetails, hasGroups, onDeleteIn, rowMarkerOffset, setSelectedRows]
    );
    ...
    const appendRow = React.useCallback(
        async (col: number, rowLocation?: number, groupId?: string): Promise<void> => {
            ...
            const doFocus = () => {
                ...
                const cell = getCellContentRef.current(getMangledCellLocation([col - rowMarkerOffset, row]));
        ...
        (args: GridMouseEventArgs) => {
            ...
            if (args.kind === "cell") {
                ...
                if (col === 0 && hasRowMarkers && rowDetails?.kind !== GridRowKind.NewRow) {
                    if ((showTrailingBlankRow && row === rows) || rowMarkers === "number" || rowSelect === "none")
                        return;
            ...
            const handleMaybeClick = (a: GridMouseCellEventArgs): boolean => {
                ...
                if (!isPrevented.current) {
                    ...
                    if (r !== undefined && r.onClick !== undefined) {
                        const mangledLocation = getMangledCellLocation(args.location);
                        const rowDetails = getGroupRowDetails(row);
                        const groupId = rowDetails?.kind === GridRowKind.GroupContent ? rowDetails.groupId : undefined;
    ...
    const onMouseMoveImpl = React.useCallback(
        (args: GridMouseEventArgs) => {
            const a: GridMouseEventArgs = {
                ...args,
                location: getMangledCellLocation([args.location[0] - rowMarkerOffset, args.location[1]]) as any,
            };
            onMouseMove?.(a);
            setScrollDir(cv => {
        ...
        React.useCallback(
            async (startIndex: number, endIndex: number) => {
                ...
                if (columnSelect !== "none") {
                    isConfirmed =
                        (await onColumnMoved?.(startIndex - rowMarkerOffset, endIndex - rowMarkerOffset)) ?? true;
                    if (isConfirmed) {
        ...
        (event: GridKeyEventArgs) => {
            const fn = async () => {
                ...
                if (event.location.col !== undefined && event.location.row !== undefined) {
                    ...
                    const renderer = getCellRenderer(markerCell);
                    const [mangledCol, mangledRow] = getMangledCellLocation([event.location.col, event.location.row]);
                    renderer?.onKeyDown?.({
                        ...event,
                        location: {
                            row: mangledRow,
    ...
    const onCopyInternal = React.useCallback(
        async (e?: ClipboardEvent, ignoreFocus?: boolean) => {
            ...
            if (hasGroups) {
                selectedRows = shiftSelection(gridSelection, 0, getMangledCellLocation, getGroupRowDetails).rows;
            }
            const copyToClipboardWithHeaders = (
                ...
                if (!copyHeaders) {
                    typeof onCopy === "function" ? onCopy(cells, e) : copyToClipboard(cells, columnIndexes, e);
                } else {
    ...
    React.useImperativeHandle(
        ...
        [appendRow, enterCellEditMode, onCopyInternal, onKeyDown, onPasteInternal, rowMarkerOffset, scrollTo]
    );
