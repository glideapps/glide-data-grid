...
import DataGridSearch, {
  DataGridSearchProps,
  DataGridSearchRef,
} from '../data-grid-search/data-grid-search';
...
import { copyToClipboard, decodeHTML, expandSelection, unquote } from './data-editor-fns';
import { DataEditorContainer } from '../data-editor-container/data-grid-container';
import { toggleBoolean } from '../data-grid/cells/boolean-cell';
import { useAutoscroll } from './use-autoscroll';
import type { CellRenderer, CustomRenderer } from '../data-grid/cells/cell-types';
...
function shiftSelection(
  ...
  let rows = input.rows;
  if (getMangledCellLocation !== undefined && getGroupRowDetails !== undefined) {
    ...
    rows = mangledSelection;
  }

  if (offset === 0 || (input.columns.length === 0 && input.current === undefined))
    return { ...input, rows };

  let current;
  if (input.current !== undefined) {
    const cellLocation: Item = [input.current.cell[0] + offset, input.current.cell[1]];
    current = {
      cell:
        getMangledCellLocation === undefined ? cellLocation : getMangledCellLocation(cellLocation),
      range: {
        ...input.current.range,
...
export interface DataEditorProps extends Props {
  ...
  /** Emitted whenever a group is expanded/collapsed.
   * @group Editing
   */
   ...
   * @group Editing
   */
  readonly onRowAppended?: (
    rowLocation?: number,
    groupId?: string
  ) => Promise<'top' | 'bottom' | 'next' | number | undefined> | void;
  ...
  readonly trailingRowOptions?: {
    ...
    /** Add trailing inside each group when there are row groups. */
    readonly inEachGroup?: boolean;
  };
...
const DataEditorImpl: React.ForwardRefRenderFunction<DataEditorRef, DataEditorProps> = (
  p,
  forwardedRef
) => {
  const [gridSelectionInner, setGridSelectionInner] =
  ...
  const isDraggable = React.useMemo(
    () =>
      onColumnMoved && onRowMoved ? true : onColumnMoved ? 'header' : onRowMoved ? 'cell' : false,
      ...
      return [
        ...
        Math.ceil(groupHeaderHeightIn * scaler),
        {
          ...
          cellHorizontalPadding:
            (themeIn?.cellHorizontalPadding ?? bt.cellHorizontalPadding) * scaler,
  ...
  const rowMarkerWidth =
    rowMarkerWidthRaw ?? (rows > 10_000 ? 48 : rows > 1000 ? 44 : rows > 100 ? 36 : 32);
  const hasRowMarkers = rowMarkers !== 'none';
  const rowMarkerOffset = hasRowMarkers ? 1 : 0;
  const showTrailingBlankRow =
    onRowAppended !== undefined && !hasGroups && trailingRowOptions !== undefined;
  ...
  const gridSelectionOuterMangled: GridSelection | undefined = React.useMemo(():
    ...
    return gridSelectionOuter === undefined
      ? undefined
      : shiftSelection(gridSelectionOuter, rowMarkerOffset);
  ...
  const setGridSelection = React.useCallback(
    (newVal: GridSelection, expand: boolean): void => {
      ...
      if (onGridSelectionChange !== undefined) {
        // TODO: ARAM - consider groups location
  ...
  const drawHeader = whenDefined(
    ...
    React.useCallback<NonNullable<typeof drawHeaderIn>>(
      (args) => {
        return (
          drawHeaderIn?.({ ...args, columnIndex: args.columnIndex - rowMarkerOffset }) ?? false
        );
      },
  ...
  const [visibleRegionY, visibleRegionTy] = React.useMemo(() => {
    return [
      scrollOffsetY !== undefined && typeof rowHeight === 'number'
        ? Math.floor(scrollOffsetY / rowHeight)
        : 0,
      scrollOffsetY !== undefined && typeof rowHeight === 'number'
  ...
  const [visibleRegion, setVisibleRegion, empty] =
    useStateWithReactiveInput<VisibleRegion>(visibleRegionInput);
  ...
  const mangledOnCellsEdited = React.useCallback<NonNullable<typeof onCellsEdited>>(
    (items: readonly EditListItem[]) => {
      const mangledItems =
        rowMarkerOffset === 0
          ...
          : items.map((x) => ({
              ...x,
              location: getMangledCellLocation([x.location[0] - rowMarkerOffset, x.location[1]]),
            }));
    ...
    [getMangledCellLocation, onCellEdited, onCellsEdited, rowMarkerOffset]
  );
  ...
  const getMangledCellContent = React.useCallback(
    ([col, row]: Item): InnerGridCell => {
      ...
      const hasRowDetails = hasGroups && rowDetails !== undefined;

      if (hasRowDetails && rowDetails.kind === GridRowKind.Group) {
        return {
      ...
      const isFirstColumn = col === rowMarkerOffset;
      const maybeFirstColumnHint = isFirstColumn ? trailingRowOptions?.hint ?? '' : '';
      const c = mangledColsRef.current[col];
      let hint = c?.trailingRowOptions?.hint ?? maybeFirstColumnHint;
      if (hasRowDetails && rowDetails.kind === GridRowKind.NewRow) {
        hint = trailingRowOptions?.hint ?? '';
      }
      ...
      if (isRowMarkerCol) {
        ...
        if (experimental?.strict === true) {
          ...
          const isSelected =
            outerCol === vr.extras?.selected?.[0] && row === vr.extras?.selected[1];
        ...
        let result = getCellContent(getMangledCellLocation([outerCol, row]));
    ...
    (val: Omit<NonNullable<typeof overlay>, 'theme'>) => {
      ...
      const groupTheme =
        column?.group !== undefined
          ? mangledGetGroupDetails(column.group)?.overrideTheme
          : undefined;
        ...
        if (initialValue !== undefined) {
          switch (content.kind) {
            case GridCellKind.Number: {
              const d = maybe(
                () => (initialValue === '-' ? -0 : Number.parseFloat(initialValue)),
                0
              );
              content = {
                ...content,
  ...
  const scrollTo = React.useCallback<ScrollToFn>(
    (col, row, dir = 'both', paddingX = 0, paddingY = 0, options = undefined): void => {
      ...
      const skipVerticalScroll =
        typeof maxScrollerHeight === 'number' && canvas.scrollHeight < maxScrollerHeight;

      const trueCol =
        typeof col !== 'number' ? (col.unit === 'cell' ? col.amount : undefined) : col;
      const trueRow =
        typeof row !== 'number' ? (row.unit === 'cell' ? row.amount : undefined) : row;
  ...
  const appendRow = React.useCallback(
    async (col: number, rowLocation?: number, groupId?: string): Promise<void> => {
      let originalGroupId = groupId;
      let originalRow = rowLocation;
      if (rowLocation !== undefined) {
        const rowDetails = getGroupRowDetails(rowLocation);
        if (rowDetails !== undefined && rowDetails.kind === GridRowKind.GroupContent) {
          originalRow = rowDetails.index;
          originalGroupId = rowDetails.groupId;
        }
      }
      ...
      const appendResult = onRowAppended?.(originalRow, originalGroupId);

      let r: 'top' | 'bottom' | 'next' | number | undefined = undefined;
      let bottom = true;
      if (appendResult !== undefined) {
        r = await appendResult;
        if (r === 'top') bottom = false;
        // If you return "next" from appendRow, it will open a new edit cell 1 row below.
        if (r === 'next' && rowLocation !== undefined) rowLocation = rowLocation + 1;
      ...
      const doFocus = () => {
        ...
        let row = typeof r === 'number' ? r : bottom ? rows : 0;

        if (rowLocation !== undefined && typeof r !== 'number') {
          row = rowLocation;
        }
  ...
  const getCustomNewRowTargetColumn = React.useCallback(
    ...
    [columnsIn, hasRowMarkers, trailingRowOptions?.targetColumn]
  );
  ...
  const handleSelect = React.useCallback(
    (args: GridMouseEventArgs) => {
      ...
      if (args.kind === 'cell') {
        ...
        if (col === 0 && hasRowMarkers && rowDetails?.kind !== GridRowKind.NewRow) {
          if (
          ...
          if (onRowMoved !== undefined) {
            ...
            const postClick = renderer?.onClick?.({
              ...
              theme: themeForCell(markerCell, mangledLocation),
          ...
          (showTrailingBlankRow && row === rows) ||
          (rowDetails !== undefined && rowDetails.kind === GridRowKind.NewRow)
        ) {
          const customTargetColumn = getCustomNewRowTargetColumn(col);
          const groupRowPosition = rowDetails !== undefined ? row : undefined;

          const groupId = rowDetails?.kind === GridRowKind.NewRow ? rowDetails.groupId : undefined;
          void appendRow(
            customTargetColumn ?? col,
            groupRowPosition !== undefined ? groupRowPosition - 1 : undefined,
            groupId
          );
        } else {
          if (cellCol !== col || cellRow !== row) {
            ...
            const startedFromLastSticky =
              ...
              gridSelection.current?.cell[1] === rows;
  ...
  const onMouseMoveImpl = React.useCallback(
    (args: GridMouseEventArgs) => {
      const a: GridMouseEventArgs = {
        ...args,
        location: getMangledCellLocation([
          args.location[0] - rowMarkerOffset,
          args.location[1],
        ]) as any,
      };
      onMouseMove?.(a);
      setScrollDir((cv) => {
        ...
        return mouseState === undefined ||
          (mouseDownData.current?.location[0] ?? 0) < rowMarkerOffset
          ? undefined
          : args.scrollEdge;
      });
    },
    [mouseState, onMouseMove, rowMarkerOffset, getMangledCellLocation]
  );
  ...
  const onVisibleRegionChangedImpl = React.useCallback(
    (
      ...
      const newRegion = {
        ...
        width: region.width,
        height:
          showTrailingBlankRow && region.y + region.height >= rows
            ? region.height - 1
            : region.height,
        tx,
        ty,
    ...
    React.useCallback(
      async (startIndex: number, endIndex: number) => {
        ...
        if (columnSelect !== 'none') {
          isConfirmed =
            (await onColumnMoved?.(startIndex - rowMarkerOffset, endIndex - rowMarkerOffset)) ??
            true;
          if (isConfirmed) {
  ...
  const onRowMovedImpl = whenDefined(
    onRowMoved,
    React.useCallback(
      (rowsIndex: number[], endIndex: number) => {
        ...
        if (isDragInSelection) {
          const startSelectedRow = endIndex - selectedRowLength + 1;
          if (endIndex > firstDragRow) {
            setSelectedRows(
              CompactSelection.fromSingleSelection([
                startSelectedRow,
                startSelectedRow + selectedRowLength,
              ]),
              undefined,
              true
            );
            ...
            setSelectedRows(
              CompactSelection.fromSingleSelection([endIndex, selectedRowLength + endIndex]),
              undefined,
              true
            );
          }
      ...
      [getGroupRowDetails, gridSelection.rows, hasGroups, onRowMoved, setSelectedRows]
    )
  );
  ...
  const updateSelectedCell = React.useCallback(
    (col: number, row: number, fromEditingTrailingRow: boolean, freeMove: boolean): boolean => {
      ...
      if (
        ...
        lastSent.current[0] === col &&
        lastSent.current[1] === row
      ) {
        lastSent.current = undefined;
      }
  ...
  const onFinishEditing = React.useCallback(
    (newValue: GridCell | undefined, movement: readonly [-1 | 0 | 1, -1 | 0 | 1]) => {
      ...
      if (gridSelection.current !== undefined && (movX !== 0 || movY !== 0)) {
        ...
        const rowDetails = getGroupRowDetails(row);
        if (
          rowDetails?.kind === undefined ||
          ![GridRowKind.Group, GridRowKind.NewRow].includes(rowDetails?.kind)
        ) {
          updateSelectedCell(
    ...
    (event: GridKeyEventArgs) => {
      const fn = async () => {
        ...
        if (event.location.col !== undefined && event.location.row !== undefined) {
          ...
          const renderer = getCellRenderer(markerCell);
          const [mangledCol, mangledRow] = getMangledCellLocation([
            event.location.col,
            event.location.row,
          ]);
          renderer?.onKeyDown?.({
        ...
        if (keybindings.selectColumn && isHotkey('ctrl+ ', event) && columnSelect !== 'none') {
          ...
          if (overlayOpen) {
            ...
            window.setTimeout(() => {
              const customTargetColumn = getCustomNewRowTargetColumn(col);
              void appendRow(customTargetColumn ?? col, row);
            }, 0);
          } else {
            onCellActivated?.([col - rowMarkerOffset, row]);
          ...
          const rowDetails = getGroupRowDetails(row);
          if (rowDetails?.kind === GridRowKind.Group || rowDetails?.kind === GridRowKind.NewRow) {
            if (shiftKey) {
              row--;
            } else {
              row++;
            ...
            if (col === 1 && shiftKey) {
              const prevRow = row - 1;
              const prevRowDetails = getGroupRowDetails(prevRow);
              if (
                prevRowDetails?.kind === GridRowKind.Group ||
                prevRowDetails?.kind === GridRowKind.NewRow
              ) {
                /**
                 * If previous row is a group or new row, we should move to the last cell of the previous row.
                 */
                row = prevRow;
              }
            } else {
      ...
      if (focused && getCellsForSelection !== undefined) {
        if (gridSelection.current !== undefined) {
          ...
          if (results.length === 1) {
            ...
            // FIXME: this is dumb
            const toCopy = results.reduce((pv, cv) =>
              pv.map((row, index) => [...row, ...cv[index]])
            );
  ...
  const disabledRows = React.useMemo(() => {
    if (showTrailingBlankRow && trailingRowOptions?.tint === true) {
  ...
  const mangledFreezeColumns = Math.min(
    mangledCols.length,
    freezeColumns + (hasRowMarkers ? 1 : 0)
  );
