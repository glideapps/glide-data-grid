...
export function drawHeader(
    ...
    if (c.alertIcon !== undefined) {
        const horizontalPosition =
            c.infoIcon !== undefined ? defaultHorizontalPosition - INFO_ICON_OFFSET : defaultHorizontalPosition;
        spriteManager.drawSprite(
            c.alertIcon,
            "error",
            ctx,
    ...
    if (isResizing) {
        walkColumns(effectiveCols, 0, translateX, 0, totalHeaderHeight, (c, x) => {
            if (c.sourceIndex === resizeCol) {
                drawColumnResizeOutline(overlayCtx, x + c.width, 0, totalHeaderHeight + 1, theme.accentColor);
                drawColumnResizeOutline(targetCtx, x + c.width, totalHeaderHeight, height, theme.accentColor);
            }
        });
    }
