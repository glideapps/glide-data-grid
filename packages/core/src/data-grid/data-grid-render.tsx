...
export function drawCell(
  ...
  const needsAnim = drawWithLastUpdate(args, cell.lastUpdated, frameTime, lastPrep, () => {
    const drawn = isInnerOnlyCell(cell)
      ? false
      : drawCustomCell?.(args as DrawArgs<GridCell>) === true;
    if (!drawn) {
  ...
  if (verticalOnly !== true) {
    ...
    while (y + translateY <= target) {
      ...
      if (
        ty >= minY &&
        ty <= maxY - 1 &&
        (!lastRowSticky || row !== rows - 1 || Math.abs(ty - stickyRowY) > 1)
      ) {
        const rowTheme = getRowThemeOverride?.(row);
        const isBottomDirection =
          dragAndDropState?.cell?.src !== undefined &&
          dragAndDropState?.cell?.src < dragAndDropState?.cell?.dest;
        const isDragAndDropRow =
          dragAndDropState?.cell?.dest === (isBottomDirection ? row - 1 : row);
...
function drawGroups(
  ...
  walkGroups(effectiveCols, width, translateX, groupHeaderHeight, (span, groupName, x, y, w, h) => {
    if (
      damage !== undefined &&
      !damage.some((d) => d[1] === -2 && d[0] >= span[0] && d[0] <= span[1])
    )
      return;
    ctx.save();
    ctx.beginPath();
    ...
    const groupTheme =
      group?.overrideTheme === undefined ? theme : { ...theme, ...group.overrideTheme };
...
export function getHeaderMenuBounds(
  x: number,
  y: number,
  width: number,
  height: number
        ...
        (drawY, row, rh, isSticky) => {
          ...
          if (isDamaged) {
            ...
            const bottom = isSticky
              ? top + rh - 1
              : Math.min(top + rh - 1, height - stickyRowHeight);
            const h = bottom - top;
    ...
    (c, drawX, colDrawStartY, clipX, startRow) => {
      ...
      if (drawRegions.length > 0) {
        ...
        for (let i = 0; i < drawRegions.length; i++) {
          ...
          if (
            intersectRect(colDrawX, colDrawY, colWidth, colHeight, dr.x, dr.y, dr.width, dr.height)
          ) {
            found = true;
            break;
          }
      ...
      walkRowsInCol(
        ...
        (drawY, row, rh, isSticky, isTrailingRow) => {
          ...
          if (rowDetails === undefined || rowDetails.kind === GridRowKind.GroupContent) {
            if (accentCount > 0 || rowDisabled) {
              ...
              if (
                ...
                fill = isSelected
                  ? blend(theme.bgSelectedSearchResult, fill)
          ...
          const hoverValue = hoverValues.find(
            (hv) => hv.item[0] === c.sourceIndex && hv.item[1] === row
          );
...
function drawFocusRing(
  ...
  if (
    ...
    !effectiveCols.some((c) => c.sourceIndex === selectedCell.current?.cell[0])
  )
    return undefined;
  ...
  const stickRowHeight =
    trailingRowType === 'sticky' && !isStickyRow ? getRowHeight(rows - 1) - 1 : 0;
...
function computeCanBlit(current: DrawGridArg, last: DrawGridArg | undefined): boolean | number {
  ...
  if (current.mappedColumns !== last.mappedColumns) {
    if (
      current.mappedColumns.length > 100 ||
      current.mappedColumns.length !== last.mappedColumns.length
    ) {
...
export function drawGrid(arg: DrawGridArg, lastArg: DrawGridArg | undefined) {
  ...
  const mustDrawFocusOnHeader =
    drawFocus && selection.current?.cell[1] === cellYOffset && translateY === 0;
  ...
  if (damage !== undefined) {
    ...
    damage = damage.filter((x) => {
      ...
      return (
        ...
        (trailingRowType &&
          intersectRect(cellXOffset, rows - 1, effectiveCols.length, 1, x[0], x[1], 1, 1))
      );
    });
    ...
    if (!isResizing && damage.length > 0) {
      ...
      if (
        ...
        damage.some(
          (x) => x[0] === selection.current?.cell[0] && x[1] === selection.current?.cell[1]
        )
      ) {
        drawFocusRing(
          targetCtx,
...
function walkRowsInCol(
  ...
  while (y < height || doSticky) {
    ...
    if (
      !isMovedStickyRow &&
      cb(y, row, rh, doingSticky, trailingRowType !== 'none' && row === rows - 1) === true
    ) {
      break;
    }
