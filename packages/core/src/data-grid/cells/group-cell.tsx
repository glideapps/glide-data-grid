...
import { getMiddleCenterBias, measureTextCached, roundedRect, clipCanvasString } from "../data-grid-lib";
...
export const groupRenderer: InternalCellRenderer<GroupCell> = {
    ...
    draw: args => {
        ...
        const countWidth = measureTextCached(`${cell.rowsCount}`, ctx, `${theme.headerFontStyle} ${theme.fontFamily}`);
        ...
        const textWidth = measureTextCached(clippedText, ctx, `${theme.headerFontStyle} ${theme.fontFamily}`);
        ...
        const circleY =
            rect.y + rect.height / 2 + getMiddleCenterBias(ctx, `${theme.headerFontStyle} ${theme.fontFamily}`) - 1;
    ...
    onClick: e => {
        if (e.onRowDetailsUpdated) {
             ...
             */
            if (posX >= iconXPosition - GROUP_ICON_CLICK_PADDING && posX <= iconXPosition + GROUP_ICON_CLICK_PADDING) {
