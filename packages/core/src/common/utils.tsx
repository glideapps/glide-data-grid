...
export const Checkmark: React.FunctionComponent<Partial<SpriteProps>> = (
  props: Partial<SpriteProps>
) => {
  const fg = props.fgColor ?? 'currentColor';
...
// I'm sorry.
const empty = Symbol();
export function useStateWithReactiveInput<T>(
  inputState: T
  ...
  return [
    inputStateRef.current[0] === empty ? state : inputStateRef.current[0],
    setStateOuter,
    onEmpty,
  ];
}
