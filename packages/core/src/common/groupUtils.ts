...
export const deleteGroupByGroupRowId = (rootGroups: RowGroup[], targetGroupRowId: string): boolean => {
    const groupToDelete = findGroupById(rootGroups, targetGroupRowId);

    const parentId = groupToDelete?.parentId ?? "";

    const parentGroup =
        parentId !== undefined && parentId !== "" ? findGroupById(rootGroups, parentId)?.groups : rootGroups;
